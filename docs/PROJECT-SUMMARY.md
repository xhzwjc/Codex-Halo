...
Codex Halo 是一个 Tauri 2 桌面额度监视器，用本机 Codex Desktop 登录态只读查询 Codex 额度，并通过 macOS 菜单栏、详情面板和悬浮窗同步展示接口当前提供的额度窗口、重置时间、重置机会和会员类型。
...
- macOS 菜单栏：按接口实际窗口直接显示紧凑的 `W` 或 `5h / W` 剩余比例，点击打开额度详情面板；Windows 使用托盘图标、tooltip 和原生菜单降级。
